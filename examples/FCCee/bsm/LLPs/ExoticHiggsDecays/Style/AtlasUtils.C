...
void myText(Double_t x,Double_t y,Color_t color, const char *text) {
  ...
  TLatex l; //l.SetTextAlign(12); l.SetTextSize(tsize); 
  l.SetTextSize(0.04);
  l.SetTextFont(42); // Edvin 2015-06-03
  l.SetNDC();
  l.SetTextColor(color);
  l.DrawLatex(x,y,text);
}

void myLine(Double_t xlo, Double_t ylo, Double_t xup, Double_t yup, Color_t color = kBlack, Width_t width = 2) {

    //printf("Welcome to myLine! \n");
    TLine l; //(xlo, ylo, xup, yup);
    l.SetLineStyle(1);
    l.SetLineWidth(width);
    l.SetLineColor(color);
    //l.SetNDC();
    l.DrawLineNDC(xlo,ylo,xup,yup);
